  ...
  _createClass(AzureStorageAdapter, [{
    ...
    value: function createFile(filename, data) {
      ...
      return new Promise(function (resolve, reject) {
        _this._client.createContainerIfNotExists(_this._container, containerParams, function (cerr, cresult, cresponse) {
          ...
          _this._client.createBlockBlobFromStream(_this._container, filename, Stream.Readable.from(data), data.length, function (err, result) {
            if (err) {
              return reject(err);
            }
    ...
    value: function getFileData(filename) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        writeable._write = function (object, encoding, done) {
          buffer = Buffer.concat([buffer, object]);
          done();
        };

        _this3._client.getBlobToStream(_this3._container, filename, writeable, function (err, result) {
          err && reject(err);
          resolve(buffer);
        });
      });
